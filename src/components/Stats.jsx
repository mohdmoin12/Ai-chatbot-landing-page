...
const Stats = () => {
  ...
  return (
    <section 
      ref={sectionRef}
      className="py-20 bg-white border-t border-gray-300"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {stats.map((stat, index) => (
            <div key={index} className="text-center group">
              <div 
                className="text-4xl md:text-5xl font-bold mb-3 transition-transform duration-300 group-hover:scale-110 text-black"
              >
                {hasAnimated ? formatNumber(counters[index], index) : '0+'}
              </div>
              <div className="text-black transition-colors duration-300 group-hover:text-gray-600">
                {stat.label}
              </div>
            </div>
          ))}
